...
void CIcGameController::RegisterChatCommands(IConsole *pConsole)
{
	...
	pConsole->Register("start_survival", "?is", CFGFLAG_SERVER, ConStartSurvival, this, "Start survival (of a certain wave)");
...
void CIcGameController::OnIcCharacterDeath(CIcCharacter *pVictim, DeathContext *pContext)
{
	...
	if(!BadReasonsToDie.Contains(DamageType) && (Killer != pVictim->GetCid()))
	{
		if(pVictim->IsHuman())
		{
			...
			if(pKiller && pKiller->IsInfected() && pKiller->GetCharacter() && DamageType != EDamageType::INFECTION_TILE)
			{
